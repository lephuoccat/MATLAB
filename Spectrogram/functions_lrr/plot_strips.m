...
% attach title to figure for identification purposes
    ...
    if (ipl == 1)
        xlabel('sample number'),ylabel('samples offset');
    else
        xlabel('seconds past offset'),ylabel('seconds offset');
    end
    title(stitle1);
    
% expand size of graphical output plot
    set(gcf,'Position',[280 278 990 660]);
