...
% Callback for button2 -- idigit: digit for plot (1-9, 10=o, 11=Z)
 function button2Callback(h,eventdata)
     ...
     if ~((idigit >= 1 && idigit <= 11))
        waitfor(errordlg('idigit must be a positive integer between 1 and 11'))
        return;
     end
     idigit=round(idigit);
     set(button2,'string',num2str(idigit));
 end
...
% plot circle for centroids
    ...
    legs=['digit:',num2str(idigit)];legend(legs);
    stitle=[' c(1) vs c(2); cep:2; vq:',num2str(vqsize),'; digit:',nums];
    title(stitle,'FontSize',20);
    
% display VQ cell information in titleBox1
