...
% plot both input and output files if iplot set to 1
%
    iplot=0;
    if (iplot == 1)
        ...
        subplot(211),plot(1:nl,x,'b');
        xlabel('time in samples'),ylabel('amplitude');
        axis tight, grid on, legend('original speech');
    
        subplot(212),plot(1:nl,y,'r');
        xlabel('time in samples'),ylabel('amplitude');
        axis tight, grid on, legend('highpass filtered speech');
    end
end
